...
void main() {
  ...
  vec3 edgeColor = uColor + vec3(0.6); 
  ...
  finalColor += vec3(uResonance) * vec3(0.8, 0.9, 1.0); 

  gl_FragColor = vec4(finalColor, 0.7 + 0.3 * fresnel);
}
`;
...
const CrystalImpactEffect = () => {
    ...
    const dummy = useMemo(() => new THREE.Object3D(), []);
    const [shards] = useState(() => Array.from({ length: 20 }, () => ({
...
const CrystalCluster: React.FC<CrystalClusterProps> = ({ position, scale, rotation, color, variant }) => {
    ...
    const geo = useMemo(() => {
        if (variant === 1) return new THREE.CylinderGeometry(0, 0.4, 4, 4); 
        if (variant === 2) return new THREE.DodecahedronGeometry(1, 0); 
        return new THREE.ConeGeometry(1, 4, 4); 
    }, [variant]);
...
const ResonantCrystal: React.FC<{position: [number, number, number]}> = ({ position }) => {
    ...
    return (
        <mesh ref={meshRef} position={position} rotation={[0, Math.PI/4, 0]}>
             <cylinderGeometry args={[0, 0.5, 6, 5]} />
             <shaderMaterial 
                ref={matRef}
                vertexShader={crystalVertexShader}
                fragmentShader={crystalFragmentShader}
...
const CrystalCavesEnvironment = () => {
    ...
    useEffect(() => {
        audio.startCrystalAmbience();
        return () => {
            audio.stopCrystalAmbience();
        };
    }, []);
    ...
    return (
        <>
            ...
            <Stars radius={150} depth={50} count={2000} factor={4} saturation={1} fade speed={0.5} />
